...
public final class AuctionDetailController {
    ...
    private void configureView() {
        ...
        bidHistoryListView.setCellFactory(lv -> new ListCell<>() {
            ...
            protected void updateItem(BidTransactionView item, boolean empty) {
                ...
                bidderLabel.setText(item.bidderName() + " placed " + CURRENCY.format(item.amount()));
                metaLabel.setText(item.timestamp().toString());
                setGraphic(content);
    ...
    private void renderAuction(AuctionView auction) {
        ...
        titleLabel.setText(auction.item().name());
        sellerLabel.setText("Seller: " + auction.sellerName());
        categoryLabel.setText(auction.item().itemType());
        typeLabel.setText(auction.item().detailLabel());
        statusValueLabel.setText(buildStatusLabel(auction));
        statusValueLabel.setStyle(buildStatusStyle(auction));
        restartCountdown();
        priceLabel.setText(CURRENCY.format(auction.item().currentPrice()));
        nextBidLabel.setText(CURRENCY.format(auction.minimumNextBid()));
        reserveLabel.setText(CURRENCY.format(auction.item().startingPrice()));
        winnerValueLabel.setText(safeValue(auction.winnerName(), "No winner yet"));
        descriptionLabel.setText(safeValue(auction.item().description(), "No description provided."));
        productHighlightLabel.setText(safeValue(auction.item().detailLabel(), ""));
        imageHintLabel.setText("Auction ID: " + auction.auctionId()
                + " | Start: " + auction.startTime().format(DATE_TIME_FORMAT)
                + " | End: " + auction.endTime().format(DATE_TIME_FORMAT));
    ...
    private void renderBidChart(AuctionView auction) {
        ...
        if (auction.bidHistory().isEmpty()) {
            series.getData().add(new XYChart.Data<>(1, auction.item().currentPrice()));
        } else {
            for (int i = 0; i < auction.bidHistory().size(); i++) {
                BidTransactionView bid = auction.bidHistory().get(i);
                series.getData().add(new XYChart.Data<>(i + 1, bid.amount()));
            }
        }
